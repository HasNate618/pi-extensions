...
- Follow the `superpowers:writing-plans` skill for plan structure: map the file structure first, right-size tasks to independently testable deliverables, and give each task concrete files, changes, and acceptance criteria. Brownfield: follow existing codebase patterns; do not unilaterally restructure, but split unwieldy files you're modifying when it's justified.
