...
export default function (pi: ExtensionAPI) {
  ...
  function setup(tui: TUI, keybindings: KeybindingsManager, config: PrefixConfig, ctx: ExtensionContext): void {
    const prefixLabel = Array.isArray(config.prefix) ? config.prefix.join("|") : config.prefix;
    ...
    ctx.ui.notify(`prefix-keys: ${prefixLabel} is now your prefix. Press it then a mapped key.`, "info");
  }
  ...
  pi.registerCommand("prefix-keys", {
    ...
    handler: async (args: string[], ctx: ExtensionContext) => {
      ...
      const lines = [
        ...
        "bindings:",
        ...Object.entries(config.bindings).map(([key, target]) => `  ${key} → ${target}`),
      ];
...
function runSelfTest(): void {
  ...
  const t: SelfTestHarness = {
    ...
    makeState(timeoutMs = 10000) {
      ...
      const state = new PrefixState(
        { prefix: "ctrl+x", timeoutMs, bindings: { m: "command:/model", T: "key:shift+tab", x: "command:/new" } },
  ...
  const cases: Array<{ name: string; run: (h: SelfTestHarness) => void | Promise<void> }> = [
    {
      name: "idle passthrough",
      run: (h) => {
        const { state } = h.makeState();
        h.check("idle: plain key passes through", state.handle("h") === undefined);
      },
    },
    {
      name: "prefix arms",
      run: (h) => {
        ...
        h.check("prefix: armed indicator shown", (indicators.at(-1) ?? "").includes("prefix ⌗"));
      },
    },
    {
      ...
      run: (h) => {
        ...
        h.check("armed: command dispatched", commands.length === 1 && commands[0] === "/model");
        h.check("armed: flash label set", (indicators.at(-1) ?? "").includes("→ /model"));
      },
    },
    {
      ...
      run: (h) => {
        ...
        h.check("armed: key target injects shift+tab bytes", r?.data === "\x1b[Z" && r?.consume !== true);
      },
    },
    {
      name: "escape cancels",
      run: (h) => {
        ...
        h.check("armed: indicator cleared on cancel", indicators.at(-1) === null);
      },
    },
    {
      ...
      run: (h) => {
        ...
        h.check("armed: unmapped key disarms without dispatch", commands.length === 0);
      },
    },
    {
    ...
    },
    {
      ...
      run: (h) => {
        ...
        h.check("array prefix: first key arms", first.handle("\x18")?.consume === true);
        const second = new PrefixState(
          { prefix: ["ctrl+x", "ctrl+g"], timeoutMs: 10000, bindings: {} },
          () => {},
          () => {},
        );
        h.check("array prefix: second key arms", second.handle("\x07")?.consume === true); // ctrl+g
      },
    },
  ];
  ...
  void (async () => {
    ...
    for (const r of results) {
      emitLine(`${r.ok ? "PASS" : "FAIL"} ${r.name}${r.detail ? ` — ${r.detail}` : ""}`);
    }
