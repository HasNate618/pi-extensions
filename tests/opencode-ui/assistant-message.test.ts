...
test("insetRenderWidth folds the base 1-char margins into the inset", () => {
	assert.equal(insetRenderWidth(20, 2), 18);
	assert.equal(insetRenderWidth(20, 1), 20);
	assert.equal(insetRenderWidth(10, 4), 4);
	assert.equal(insetRenderWidth(3, 4), 1);
});

test("insetRenderedLines insets text lines 2 cells each side", () => {
	// inner = 20 - 4 = 16, base rendered at insetRenderWidth(20, 2) = 18
	const base = [baseLine(18, "hello world"), baseLine(18, "second line")];
	const out = insetRenderedLines(base, 20, 2);
...
test("insetRenderedLines never overflows the row", () => {
	...
	for (const line of insetRenderedLines(base, 20, 2)) {
...
test("insetRenderedLines with inset=1 keeps a 1-cell gutter", () => {
	...
	const out = insetRenderedLines(base, 20, 1);
